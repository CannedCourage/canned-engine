...
AssetManager::AssetManager( Graphics& g ) : log( "AssetManager" ), graphics( g )
{
}
...
void AssetManager::BuildMesh( const aiScene* sc )
{
	...
	AccquireMeshResources();
}

void AssetManager::AccquireMeshResources( void )
{
	for( unsigned int m = 0; m < meshAssets.size(); ++m )
	{
		log.Message( "Accquiring Resources" );

		Mesh& mesh = meshAssets[m];
...
void AssetManager::ReleaseMeshResources( void )
{
	for( unsigned int m = 0; m < meshAssets.size(); ++m )
	{
