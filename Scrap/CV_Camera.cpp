#include "Camera/CV_Camera.h"
#include "Maths/Vector.h"
#include <math.h>
#include "Camera/ControlConstants.h"
#include "Graphics/Screen.h"
#include "Camera/Metrics.h"
...
CCamera::CCamera()
{
    ...
    m_maxVelocity     = PLAYERSPEED;
    m_invertY         = FALSE;
    m_enableYMovement = TRUE;
    m_position        = D3DXVECTOR3( 0.0f, 0.0f, 0.0f );
    m_velocity        = D3DXVECTOR3( 0.0f, 0.0f, 0.0f );
    m_look            = D3DXVECTOR3( 0.0f, 0.0f, 1.0f );
    CreateProjectionMatrix( D3DXToRadian(45), (FLOAT)SCREEN_WIDTH/(FLOAT)SCREEN_HEIGHT, 0.1f, 1000.0f );
    Update();
} 
...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void CCamera::ProcessInput( long xDelta, long yDelta, long zDelta, float elapsedTime )
{
	...
	Yaw( (mousePos.x - mouseX) * elapsedTime * 0.001f); //1.8f
    Pitch( (mousePos.y - mouseY) * elapsedTime * 0.001f ); //1.8f

	::SetCursorPos(SCREEN_WIDTH/2,SCREEN_HEIGHT/2);

	if( ::GetAsyncKeyState(KEY_LEFT) & 0x8000f )
	{
		Strafe( -cameraSpeed * elapsedTime );
	}
	if( ::GetAsyncKeyState(KEY_RIGHT) & 0x8000f )
	{
		Strafe( cameraSpeed * elapsedTime );
	}
	if( ::GetAsyncKeyState(KEY_FORWARD) & 0x8000f )
	{
		MoveForward( cameraSpeed * elapsedTime );
	}
	if( ::GetAsyncKeyState(KEY_BACKWARD) & 0x8000f )
	{
		MoveForward( -cameraSpeed * elapsedTime );
	}
}
...
/*
	//Yaw( xDelta * elapsedTime * 0.001f );
	//Pitch( yDelta * elapsedTime * 0.001f );

	if( ::GetAsyncKeyState(KEY_UP) & 0x8000f )
	{
		Up( ControlSpeed * 100 );
	}
	if( ::GetAsyncKeyState(KEY_DOWN) & 0x8000f )
	{
		Down( ControlSpeed * 100 );
	}
/*
